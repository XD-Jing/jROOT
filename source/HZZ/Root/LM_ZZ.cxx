...
#include <TCanvas.h>
    ...
    LM_ZZ::LM_ZZ(std::string treename, std::string outfile, std::string outopt, bool THEO, bool SYST)
: tree(0), treename(treename), SYST(SYST), THEO(THEO)
{
    ...
    reader->BookMVA( "BDTG", "/afs/cern.ch/work/l/liji/public/jROOT/source/HZZ/marco/TMVAClassification_BDTG.weights.xml");
}
...
void LM_ZZ::Close(){
    ...
    for (auto h : llvv_VBF)  h.second->Write();
    for (auto h : eevv_VBF)  h.second->Write();
    for (auto h : mmvv_VBF)  h.second->Write();
    fout->Close();
}

bool LM_ZZ::LoopROOT(std::string filename, float xsec){
...
bool LM_ZZ::mkHistVar(std::string var, int nbins, double left, double right){
    ...
    llvv_VBF[std::make_pair(this->treename, var )] = new TH1F(("llvv__VBF__"+var+"__"+this->treename).c_str(), "", nbins, left, right);
    eevv_VBF[std::make_pair(this->treename, var )] = new TH1F(("eevv__VBF__"+var+"__"+this->treename).c_str(), "", nbins, left, right);
    mmvv_VBF[std::make_pair(this->treename, var )] = new TH1F(("mmvv__VBF__"+var+"__"+this->treename).c_str(), "", nbins, left, right);
    for (auto theo:inVarTHEO){
        ...
        llvv_VBF[std::make_pair(theo.first, var)] = new TH1F(("llvv__VBF__"+var+"__"+theo.first).c_str(), "", nbins, left, right);
        eevv_VBF[std::make_pair(theo.first, var)] = new TH1F(("eevv__VBF__"+var+"__"+theo.first).c_str(), "", nbins, left, right);
        mmvv_VBF[std::make_pair(theo.first, var)] = new TH1F(("mmvv__VBF__"+var+"__"+theo.first).c_str(), "", nbins, left, right);
    }
    for (auto syst:inVarSYST){
        ...
        llvv_VBF[std::make_pair(syst.first, var)] = new TH1F(("llvv__VBF__"+var+"__"+syst.first).c_str(), "", nbins, left, right);
        eevv_VBF[std::make_pair(syst.first, var)] = new TH1F(("eevv__VBF__"+var+"__"+syst.first).c_str(), "", nbins, left, right);
        mmvv_VBF[std::make_pair(syst.first, var)] = new TH1F(("mmvv__VBF__"+var+"__"+syst.first).c_str(), "", nbins, left, right);
    }
    return true;
}
    ...
    if (region == RG::_VBF_MM){
        //if (fabs(fEvt["weight_gen"])<=100){
            for (auto var: histVars){
                ...
                llvv_VBF[std::make_pair(this->treename, var)]->Fill(fEvt[var], fEvt["weight"] * this->xsec);
                mmvv_VBF[std::make_pair(this->treename, var)]->Fill(fEvt[var], fEvt["weight"] * this->xsec);
                for (auto theo:inVarTHEO){
                    ...
                    llvv_VBF[std::make_pair(theo.first, var)]->Fill(fEvt[var], fEvt[theo.second] * sf);
                    mmvv_VBF[std::make_pair(theo.first, var)]->Fill(fEvt[var], fEvt[theo.second] * sf);
                }
                for (auto syst:inVarSYST){
                    ...
                    llvv_VBF[std::make_pair(syst.first, var)]->Fill(fEvt[var], fEvt[syst.second] * this->xsec);
                    mmvv_VBF[std::make_pair(syst.first, var)]->Fill(fEvt[var], fEvt[syst.second] * this->xsec);
                }
            }
        //}
    }
    ...
    if (region == RG::_VBF_EE){
        //if (fabs(fEvt["weight_gen"])<=100){
            for (auto var: histVars){
                ...
                llvv_VBF[std::make_pair(this->treename, var)]->Fill(fEvt[var], fEvt["weight"] * this->xsec);
                eevv_VBF[std::make_pair(this->treename, var)]->Fill(fEvt[var], fEvt["weight"] * this->xsec);
                for (auto theo:inVarTHEO){
                    llvv_GGH[std::make_pair(theo.first, var)]->Fill(fEvt[var], fEvt[theo.second] * sf);
                    eevv_GGH[std::make_pair(theo.first, var)]->Fill(fEvt[var], fEvt[theo.second] * sf);
                    llvv_VBF[std::make_pair(theo.first, var)]->Fill(fEvt[var], fEvt[theo.second] * sf);
                    eevv_VBF[std::make_pair(theo.first, var)]->Fill(fEvt[var], fEvt[theo.second] * sf);
                }
                for (auto syst:inVarSYST){
                    llvv_GGH[std::make_pair(syst.first, var)]->Fill(fEvt[var], fEvt[syst.second] * this->xsec);
                    eevv_GGH[std::make_pair(syst.first, var)]->Fill(fEvt[var], fEvt[syst.second] * this->xsec);
                    llvv_VBF[std::make_pair(syst.first, var)]->Fill(fEvt[var], fEvt[syst.second] * this->xsec);
                    eevv_VBF[std::make_pair(syst.first, var)]->Fill(fEvt[var], fEvt[syst.second] * this->xsec);
                }
            }
        //}
    }
