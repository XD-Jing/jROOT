...
#include <TCanvas.h>
    ...
    LM_emuCR::LM_emuCR(std::string treename, std::string outfile, std::string outopt, bool THEO, bool SYST)
...
bool LM_emuCR::LoopROOT(std::string filename, float xsec, std::string treename){
...
bool LM_emuCR::mkHistVar(std::string var, int nbins, double left, double right){
...
int LM_emuCR::Cut()
{
    ...
    if (iEvt["event_3CR"]!=0)                     return LM_emuCR_Regions::_NONE;
    if (!(fEvt["M2Lep"]>76 && fEvt["M2Lep"]<106)) return LM_emuCR_Regions::_NONE;
    if (fEvt["met_tst"]<=90)                      return LM_emuCR_Regions::_NONE;
    if (iEvt["n_bjets"]!=0)                       return LM_emuCR_Regions::_NONE;
    if (fabs(fEvt["dLepR"])>=1.8)                 return LM_emuCR_Regions::_NONE;
    if (fEvt["met_signif"]<=9)                    return LM_emuCR_Regions::_NONE;

    if (iEvt["event_type"]==0)         return LM_emuCR_Regions::_MM;
    if (iEvt["event_type"]==1)         return LM_emuCR_Regions::_EE;
    if (iEvt["event_type"]==2)         return LM_emuCR_Regions::_EM;

    return LM_emuCR_Regions::_NONE;
}

void LM_emuCR::LoopEVT(int region)
{
    ...
    if (region == LM_emuCR_Regions::_MM){
    ...
    if (region == LM_emuCR_Regions::_EE){
        for (auto var: histVars){
            hist_llvv[std::make_pair(this->treename, var)]->Fill(fEvt[var], sf);
    ...
    if (region == LM_emuCR_Regions::_EM){
